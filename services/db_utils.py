...
# ノート保存
# -------------------------
def save_note(kind: str, text: str, image_bytes: bytes = None, user_id: str = "global"):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""
        CREATE TABLE IF NOT EXISTS notes (
    ...
    """)
    c.execute("""
        INSERT INTO notes (user_id, kind, text, image, created_at)
        VALUES (?, ?, ?, ?, ?)
    """, (
        user_id,
        kind,
...
# 実習モード用ノート保存
# -------------------------
def save_note(kind: str, text: str, image_bytes: bytes = None, user_id: str = "global", tags: list[str] = None, ref_link: str = None):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""
        CREATE TABLE IF NOT EXISTS notes (
    ...
    """)
    c.execute(
        "INSERT INTO notes (user_id, kind, text, image, tags, ref_link) VALUES (?, ?, ?, ?, ?, ?)",
        (user_id, kind, text, image_bytes, json.dumps(tags or [], ensure_ascii=False), ref_link)
    )
    conn.commit()
    conn.close()
...
# 実習モード用ノート一覧取得
# -------------------------
def list_notes(user_id: str = "global", limit: int = 100):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""
...
def save_nickname(user_id: str, nickname: str):
    """
    指定したユーザーIDのニックネームを保存（更新）
    """
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute("""
