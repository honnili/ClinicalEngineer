...
def render():
    ...
    # ノートタイプ
    note_kind = st.radio("ノートタイプ", ["自由ノート", "罫線ノート", "方眼ノート", "日報テンプレート"], horizontal=True)

    # テキストメモ
    text = st.text_area("テキストメモ", height=120, placeholder="重要点・手順・注意事項など")

    # キャンバス（色分け・太さ・けしごむはツールバーで変更）
    ...
    with col1:
        if st.button("保存"):
            ...
            save_note(kind=_map_kind(note_kind), text=text, image_bytes=img_bytes)
            st.success("保存しました。")
...
def _map_kind(kind):
    return {"自由ノート":"free","罫線ノート":"ruled","方眼ノート":"grid","日報テンプレート":"report"}[kind]
